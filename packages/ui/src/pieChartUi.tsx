...
export default function DataCard({ title, initialData, alternateData }: DataCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-4 w-auto mb-4">
        ...
        {alternateData && (
          <label className="inline-flex items-center cursor-pointer">
            ...
            <div className="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-green-500 relative transition-all duration-300">
              <div className={`absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 ${enabled ? "translate-x-5" : ""}`} />
            </div>
          </label>
        )}
      </div>
      ...
      {/* Summary Row */}
      <div className="flex justify-between text-sm gap-4 text-gray-700 mt-3">
